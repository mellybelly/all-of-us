...
tiers), drafting an initial taxonomy, then validating against an independent
~100-record sample.
...
A few judgment calls came up during taxonomy drafting where the right choice
depends on what comparison against IC strategic plans you want to enable.
Flagging them now:

1. **Dermatology over-representation.** Dermatology is ~5% of the corpus but
   that's driven by 1–2 prolific groups doing rapid-fire AoU case-control
   epi. If the goal is "where does AoU connect to NIH IC missions," the IC
   answer is mostly NIAMS (small budget) and NCI (skin cancer). Want me to
   keep Dermatology as its own theme, fold into Autoimmune+Cancer, or fold
   into "Specialty disease epi"?

2. **Wearables: own theme or subtheme of Methods?** I split wearables (theme
   7) from Methods/Infrastructure (theme 3) because the data-source
   distinction is large. But many records could land in either. If you'd
   prefer wearables-as-method-subtheme that's a one-line change.

3. **Pharmacogenomics: own theme or subtheme of Genomics?** Currently its
   own theme (16). Could fold into theme 2 as a fifth subtheme. Kept
   separate because the IC alignment is distinctive (NIGMS PGRN, NHLBI for
   CV drugs, NCI for cancer drugs) — but the corpus share is small.

4. **ELSI / engagement: own theme or "non-research" exclusion?** Some of the
   17 records in theme 17 are AoU-meta pubs (e.g., NEJM 2019 program
   announcement); some are real ELSI research. Want me to split these into
   "AoU-program-meta" (exclude from substantive-research counts) vs "ELSI
   research" (count)?

5. **Stroke positioning.** Stroke appears in three themes (1
   cardiometabolic when cardio-embolic; 11 aging when in older-adult
   cohorts; 14 neurology when framed as cerebrovascular). I lean toward
   tagging all stroke records with both Cardiometabolic and Neurology and
   letting multi-tag do the work. Confirm?

6. **CKM Syndrome.** The "Cardiovascular-Kidney-Metabolic syndrome" framing
   is recent and rising fast in the corpus (NIH released the CKM advisory in
   2023). Currently rolled into Cardiometabolic (1.3). Want it as its own
   subtheme for trackability against the NHLBI/NIDDK strategic plans?

7. **AoU-meta / program-introduction pubs.** ~25 pubs are basically *about*
   AoU rather than *using* AoU data ("The All of Us Research Program," "AoU
   year in review 2023-2024," interviews with the CEO). These currently sit
   in theme 17. Should they be in their own audit-only bucket?
